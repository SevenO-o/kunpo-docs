...
用户长期约定：**API 文档主要给 AI 阅读，用于生成正确的接入代码。** 本约定适用于后续所有页面，不限于 VS 2.5。

- 每个模型页明确可调用模型 ID、Base URL、鉴权、方法、路径、Content-Type、请求与响应字段路径。
- 参数注明 JSON 类型、必填条件、默认值、枚举、范围、单位、互斥或依赖条件；别名与优先级必须明确，不能用“参考官方文档”替代平台实际约束。
- 异步接口写明公开 ID、非终态、成功和失败终态、结果位置、查询频率、超时恢复、POST 重试与幂等边界。不同协议的响应不得混写。
- 计费写明币种、单位、价格分档、公式、输入与输出计算、分组倍率、预扣与结算语义；不把示例账单或全零占位字段当成价格规则。
- 示例可直接复制执行，仅凭环境变量或明确占位符替换凭据与用户素材。禁止真实密钥、用户子应用 ID、私人素材链接。
- 明确平台已开放能力及 CDN 保存行为，不将供应商原始能力、尚未开放接口或未确认限制当成平台契约。
- 继续以 MDX 为单一信息源。若未来添加 OpenAPI / JSON Schema，须与页面共用可验证来源或同步校验，避免维护两份相互矛盾的参数表。
- `npm run docs:export` 自动生成公开 `/llms.txt` 与导航页面 `.md`。只允许从 `docs.json` 已公开页面导出，并展开其显式引用的 `snippets/`；禁止打包整个源码目录或内部维护文档。不要手改导出的 Markdown。
